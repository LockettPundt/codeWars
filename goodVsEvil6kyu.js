...
// of evil. Different races will certainly be involved. Each race has a certain worth when
// battling against others. On the side of good we have the following races, with their
// associated worth:
...
// add up the worth of the side of good and compare it with the worth of the side of
...
function goodVsEvil(good, evil) {
  ...
  const goodNum = good.split(' ').reduce((acc, num, index) => acc += Object.values(goodSide[index])[0] * parseInt(num), 0);
  const evilNum = evil.split(' ').reduce((acc, num, index) => acc += Object.values(evilSide[index])[0] * parseInt(num), 0);
  if (goodNum === evilNum) return 'Battle Result: No victor on this battle field';
  return goodNum > evilNum
