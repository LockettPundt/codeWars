// Step 1: Create a function called encode() to
...
const encode = (string) => {
  ...
  return string.split('').map((letter) => (vowels.includes(letter)
    ? `${vowels.indexOf(letter) + 1}` : letter)).join('');
};

const decode = (string) => {
  const vowels = ['a', 'e', 'i', 'o', 'u'];
  return string.split('').map((letter) => (parseInt(letter)
    ? vowels[parseInt(letter) - 1] : letter)).join('');
};
