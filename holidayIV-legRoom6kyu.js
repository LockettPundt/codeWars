...
function legRoom(a, b) {
  if (b.split('00').length > 1) return 'Jackpot!';
  const legLength = Math.floor(a * 0.55);
  const consonants = b.match(/[^aeiou]/gi).length * 2;
  const seatRating = Math.floor(consonants / legLength * 100);
  if (seatRating >= 25) {
    return 'super comfy';
  } if (seatRating <= 15) {
    return 'ouch';
